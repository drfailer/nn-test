...
template <CostFunction Cost, ActivationFunction Act, OptimizeFunction Opt>
class Trainer {
  public:
    Trainer(Model *model, Tracer *tracer = nullptr)
        : model_(model), tracer_(tracer) {}

  public:
    Vector act(Vector const &z) const { return map(activation_, z); }
    ...
    Vector compute_z(Layer const &layer, Vector const &a) const {
        ...
        gemv<ftype>(CblasNoTrans, layer.nb_nodes, layer.nb_inputs, 1.0,
                    layer.weights.mem, layer.nb_inputs, a.mem, 1, 1.0, z.mem,
                    1);
        return z;
    }
    ...
    ftype evaluate_cost(DataSet const &ds) const {
        ftype cost_sum = 0;
        ...
        for (auto const &elt : ds) {
            ...
            cost_sum +=
                std::accumulate(costs.mem, costs.mem + costs.size, 0.0) /
                costs.size;
        }
        return cost_sum / ds.size();
    }

    ftype evaluate_accuracy(DataSet const &ds) const {
    ...
    std::pair<ftype, ftype> evaluate(DataSet const &ds) const {
        size_t count_valid = 0;
        ftype cost_sum = 0;
        ftype avg_cost = 0;
        ftype accuracy = 0;
